    ...
    for page in range(1,5):
        ...
        if res['issuccess'] !=1:
            ...
            for feedback in feedbacks:
                ...
                #加入数据库
                pollMysql(goodId,feedbackTitle,feedbackPicture,feedbacktime,goodNum
                          )
                # 写入文件
...
#加入数据库
def pollMysql(good_id,feedbackTitle,feedbackPicture,feedbackTime,feedbackId_id):
    good_sql=SqlHelper(host='localhost',port=3306,db='etsy',user='root',password='123456')
    good_one='insert into platformes_feedbacks(good_id,feedbackTitle,feedbackPicture,feedbackTime,feedbackId_id) value(%s,%s,%s,%s,%s)'
    good_one_name=(good_id,feedbackTitle,feedbackPicture,feedbackTime,feedbackId_id)
